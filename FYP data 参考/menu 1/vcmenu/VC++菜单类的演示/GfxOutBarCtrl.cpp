...
// Use & modify as you want & need, and leave those lines.
// 本代码供免费使用，使用时请遵守如上协定

// GfxOutBarCtrl.cpp : CGfxOutBarCtrl类源文件
//
...
{
	crBackGroundColor =RGB(58,110,180);//GetSysColor(COLOR_BACKGROUND); 
	crBackGroundColor1 = RGB(58,110,180);//GetSysColor(COLOR_BACKGROUND);
	...
	iFolderHeight = 22;//22 modify by yh
...
void CGfxOutBarCtrl::DrawItem(CDC * pDC, const int iFolder, CRect rc, const int index, const bool bOnlyImage)
{
	...
	if (IsSmallIconView())
	{
		if (ima)
		{
			...
			CSize szImage = CRect(ii.rcImage).Size();
//			CSize szImage;
//			szImage.cx=CRect(ii.rcImage).Width()+10;
//			szImage.cy=CRect(ii.rcImage).Height()+10;
			CPoint pt;
			pt.x = rc.left + 2;
			pt.y = rc.top + (rc.Height() - szImage.cy) / 2;
...
//				rc.top += szImage.cy + yLargeIconLabelOffset;
				rc.top += szImage.cy + yLargeIconLabelOffset;	//zdy 
...
void CGfxOutBarCtrl::GetIconRect(const int iFolder, const int iIndex, CRect & rect)
{
	...
	{
		rect.SetRect(rc.left + (rc.Width() - sz.cx)/2 - 5, top, rc.left + (rc.Width() - sz.cx)/2 + sz.cx+5, top + sz.cy+10);
...
void CGfxOutBarCtrl::Refresh_QQ()
{
	CRect clientRc;
	GetClientRect(&clientRc);
	CClientDC dc(this);
